        ...
        if path == "/tool/video-downloader":
            ...
            <main class="wrap">
              ...
              <section class="card" style="margin-top:18px">
                ...
                <p class="mini">A chave fica salva apenas neste PC, em <code>userdata/config.json</code>. Ela é usada para chamar o Whisper/OpenAI.</p>
                <div class="row">
                  <div>
                  ...
                  <button onclick="saveSettings()">Salvar chave</button>
                </div>
              ...
              <section class="card" style="margin-top:18px">
                ...
                <div class="actions">
                  ...
                  <div>
                    <h3>Transcrição</h3>
                    <p class="mini">Usa Whisper/OpenAI. Configure a chave abaixo uma vez e gere TXT, MD e JSON.</p>
...
async function loadSettings() {{
  ...
  document.getElementById('settingsStatus').textContent = cfg.openai_configured ? 'Chave configurada: ' + cfg.openai_key_masked : 'Nenhuma chave configurada ainda.';
}}
async function saveSettings() {{
  const key = document.getElementById('openaiKey').value.trim();
  const base = document.getElementById('openaiBase').value.trim();
  const data = await settingsApi({{action:'save', openai_api_key:key, openai_base_url:base}});
  ...
  box.textContent = data.message + '\n' + JSON.stringify(data.data || {{}}, null, 2);
    ...
    def do_POST(self):
        ...
        if path == "/api/settings":
            ...
            if action == "save":
                patch = {"openai_base_url": payload.get("openai_base_url", "https://api.openai.com/v1").strip() or "https://api.openai.com/v1"}
