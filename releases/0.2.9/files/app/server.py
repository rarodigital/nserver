    ...
    def do_POST(self):
        ...
        if path == "/api/video":
            ...
            if result.ok and data.get("downloadable") and file_path:
                ...
                data["download_url"] = f"/download/{token}"
                data["download_note"] = "Link temporário. Se o destino for dispositivo, o arquivo é removido do servidor após o download." if data.get("delete_after_download") else "Link para baixar uma cópia; o arquivo continua salvo no Nserver."
            save_history({
                "date": time.strftime("%Y-%m-%d %H:%M:%S"),
                "url": payload.get("url", ""),
                "operation": payload.get("action", ""),
                "status": "ok" if result.ok else "erro",
                "location": data.get("folder") or data.get("file") or "",
            })
