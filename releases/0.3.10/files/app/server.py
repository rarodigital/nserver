        ...
        if path == "/tool/video-editor":
            ...
            <main class="wrap">
              <section class="card">
                ...
                <div class="meta">
                  <button id="tabCut" onclick="showTab('cut')">1. Cortar vídeo</button>
                  <button id="tabCaption" class="button secondary" onclick="showTab('caption')">2. Colocar legenda</button>
                </div>
              </section>
              ...
              <section id="cutPanel" class="card" style="margin-top:18px">
                ...
                <div class="row">
                  <select id="sourceType"><option value="library">Biblioteca</option><option value="upload">Upload</option><option value="url">URL / Drive / YouTube</option></select>
                  <select id="source"></select>
                  <input id="uploadFile" type="file" />
                  <input id="sourceUrl" placeholder="Cole o link do vídeo" />
                  <button onclick="prepareSource()">Selecionar vídeo</button>
                </div>
                <p class="mini">As 3 entradas ficam visíveis: selecione o tipo e use Biblioteca, arquivo do dispositivo ou link.</p>
              ...
              <section id="cutControls" class="card" style="margin-top:18px">
                ...
                <div class="row">
                  <button onclick="applyPreset('dry')">Vitor/Reels seco</button>
                  <button class="button secondary" onclick="applyPreset('balanced')">Equilibrado</button>
                  <button class="button secondary" onclick="applyPreset('safe')">Conservador</button>
                </div>
                <div class="row">
                  <div><label>Noise</label><input id="noise" value="-32dB"></div>
                  <div><label>Min silêncio</label><input id="minSilence" value="0.25"></div>
                  <div><label>Pad antes</label><input id="padBefore" value="0.04"></div>
                  <div><label>Pad depois</label><input id="padAfter" value="0.04"></div>
                ...
                <div class="row">
                  ...
                  <button class="button secondary" onclick="previewKeeps()">Executar preview</button>
              ...
              <section id="timelineCard" class="card" style="margin-top:18px">
                ...
                <p class="mini">Verde = manter. Vermelho = cortar. Edite os tempos no JSON ou use Liberar/Cortar trecho no item selecionado.</p>
                <div id="timeline" style="display:flex;height:34px;border:1px solid #263244;border-radius:12px;overflow:hidden;background:#111827"></div>
                ...
                <label>cuts.json — fonte da verdade</label>
              ...
              <section id="captionPanel" class="card" style="margin-top:18px;display:none">
                ...
                <div class="row">
                  <button onclick="useCurrentVideoForCaption()">Usar vídeo atual</button>
                  <select id="captionSourceType"><option value="library">Biblioteca</option><option value="upload">Upload</option><option value="url">URL / Drive / YouTube</option></select>
                  <select id="captionSource"></select>
                  <input id="captionUpload" type="file" />
                  <input id="captionUrl" placeholder="Cole o link do vídeo para legendar" />
                  <button onclick="prepareCaptionSource()">Preparar vídeo da legenda</button>
                </div>
...
function toggleSource(){return true;}
async function loadSources(){const data=await editorApi({action:'list_sources'}); SOURCES=(data.data&&data.data.items)||[]; const opts=SOURCES.map(x=>`<option value="${esc(x.relative)}">${esc(x.name)} — ${esc(x.modified)}</option>`).join(''); document.getElementById('source').innerHTML=opts; document.getElementById('captionSource').innerHTML=opts; document.getElementById('result').textContent=SOURCES.length?'Escolha um vídeo, envie upload ou cole uma URL.':'Nenhum vídeo encontrado. Use upload ou URL.';}
...
async function prepareSource(){const box=document.getElementById('result'); try{let rel=''; const t=document.getElementById('sourceType').value; if(t==='upload'){box.textContent='Enviando upload...'; rel=await uploadFile(); document.getElementById('sourceType').value='library'; toggleSource(); document.getElementById('source').value=rel;} else if(t==='url'){box.textContent='Baixando URL para a Biblioteca...'; const data=await videoApi({action:'download_video',url:document.getElementById('sourceUrl').value,destination:'server',quality:'720',format:'mp4'}); if(!data.ok) throw new Error(data.message); await loadSources(); rel=(data.data&&data.data.file)?data.data.file.split('/midias/').pop():''; if(rel) document.getElementById('source').value=rel; document.getElementById('sourceType').value='library'; toggleSource();} else rel=document.getElementById('source').value; const p=await editorApi({action:'probe',source:rel}); if(!p.ok) throw new Error(p.message); CURRENT=p.data; document.getElementById('player').src=p.data.play_url||''; document.getElementById('captionSource').value=rel; document.getElementById('captionPlayer').src=p.data.play_url||''; box.textContent='Vídeo selecionado: '+JSON.stringify(p.data,null,2);}catch(err){box.textContent='Erro: '+err;}}
...
async function prepareCaptionSource(){const box=document.getElementById('captionResult'); try{let rel=''; const t=document.getElementById('captionSourceType').value; if(t==='upload'){box.textContent='Enviando upload...'; rel=await uploadCaptionFile(); document.getElementById('captionSource').value=rel;} else if(t==='url'){box.textContent='Baixando URL para a Biblioteca...'; const data=await videoApi({action:'download_video',url:document.getElementById('captionUrl').value,destination:'server',quality:'720',format:'mp4'}); if(!data.ok) throw new Error(data.message); await loadSources(); rel=(data.data&&data.data.file)?data.data.file.split('/midias/').pop():''; if(rel) document.getElementById('captionSource').value=rel;} else rel=document.getElementById('captionSource').value; const p=await editorApi({action:'probe',source:document.getElementById('captionSource').value}); if(!p.ok) throw new Error(p.message); document.getElementById('captionPlayer').src=p.data.play_url||''; box.textContent='Vídeo da legenda pronto: '+p.data.filename;}catch(err){box.textContent='Erro ao preparar legenda: '+err;}}
function applyPreset(p){const vals={dry:['-32dB','0.25','0.04','0.04','0.12','0.18'],balanced:['-35dB','0.35','0.08','0.08','0.18','0.25'],safe:['-38dB','0.50','0.12','0.14','0.25','0.35']}[p]; ['noise','minSilence','padBefore','padAfter','ignoreCut','minKeep'].forEach((id,i)=>document.getElementById(id).value=vals[i]);}
function sourceRel(){return document.getElementById('source').value;}
async function analyzeCuts(){const box=document.getElementById('result'); try{box.textContent='Analisando silêncios...'; const data=await editorApi({action:'analyze_cuts',source:sourceRel(),noise:noise.value,min_silence:minSilence.value,pad_before:padBefore.value,pad_after:padAfter.value,ignore_cut_under:ignoreCut.value,min_keep:minKeep.value}); if(!data.ok) throw new Error(data.message); CUTS=data.data.cuts; renderCuts(); box.textContent=data.message;}catch(err){box.textContent='Erro ao analisar: '+err;}}
function getCuts(){try{return JSON.parse(document.getElementById('cutsJson').value);}catch(e){alert('cuts.json inválido'); throw e;}}
function setAction(id,action){CUTS=getCuts(); const it=CUTS.items.find(x=>x.id===id); if(it){it.action=action; it.reason=action==='keep'?'manual: liberar trecho':'manual: cortar trecho'; renderCuts();}}
function renderCuts(){document.getElementById('cutsJson').value=JSON.stringify(CUTS,null,2); const dur=Number(CUTS.source_duration||1); const tl=document.getElementById('timeline'); tl.innerHTML=(CUTS.items||[]).map(x=>`<div title="${esc(x.id)} ${esc(x.action)}" style="width:${Math.max(.5,(x.end-x.start)/dur*100)}%;background:${x.action==='keep'?'#22c55e':'#ef4444'};border-right:1px solid #111"></div>`).join(''); document.getElementById('cutItems').innerHTML=(CUTS.items||[]).map(x=>`<div class="media-item"><div><b>${esc(x.action)}</b><br><span class="mini">${x.start}s → ${x.end}s</span></div><div><b>${esc(x.id)}</b><p class="mini">${esc(x.reason||'')}</p><div class="item-actions"><button onclick="setAction('${esc(x.id)}','keep')">Liberar trecho</button><button class="danger-btn" onclick="setAction('${esc(x.id)}','cut')">Cortar trecho</button></div></div></div>`).join('');}
function previewKeeps(){CUTS=getCuts(); const keeps=(CUTS.items||[]).filter(x=>x.action==='keep'); const v=document.getElementById('player'); let i=0; if(!keeps.length)return; clearInterval(PREVIEW_TIMER); function playSeg(){if(i>=keeps.length){v.pause();return;} v.currentTime=Number(keeps[i].start||0); v.play();} PREVIEW_TIMER=setInterval(()=>{if(i<keeps.length && v.currentTime>=Number(keeps[i].end||0)){i++;playSeg();}},120); playSeg();}
function pausePreview(){clearInterval(PREVIEW_TIMER); document.getElementById('player').pause();}
async function renderFinal(){const box=document.getElementById('result'); try{CUTS=getCuts(); box.textContent='Renderizando final...'; const data=await editorApi({action:'render_cuts',source:sourceRel(),cuts:CUTS,title:(CURRENT&&CURRENT.filename)||'cutflow'}); if(!data.ok) throw new Error(data.message); LAST_FINAL=data.data.relative; box.textContent=data.message+'\n'+JSON.stringify(data.data,null,2); if(data.data.play_url) document.getElementById('player').src=data.data.play_url; await loadSources();}catch(err){box.textContent='Erro ao exportar: '+err;}}
...
loadSources(); toggleSource(); renderCuts();
</script>
            """, authenticated=True)
            self.send_html(body)
            return
...
loadLibrary();
    ...
    def do_POST(self):
        ...
        if path == "/api/editor":
            ...
            data = result.data or {}
            if result.ok:
                rel = data.get("source") or payload.get("source") or ""
                try:
                    if rel:
