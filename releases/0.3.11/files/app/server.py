...
async function prepareSource(){const box=document.getElementById('result'); try{let rel=''; const upload=document.getElementById('uploadFile'); const url=(document.getElementById('sourceUrl').value||'').trim(); let t=document.getElementById('sourceType').value; if(url) t='url'; else if(upload.files&&upload.files.length) t='upload'; if(t==='upload'){box.textContent='Enviando upload...'; rel=await uploadFile(); document.getElementById('source').value=rel;} else if(t==='url'){box.textContent='Baixando URL para a Biblioteca...'; const data=await videoApi({action:'download_video',url,destination:'server',quality:'720',format:'mp4'}); if(!data.ok) throw new Error(data.message); await loadSources(); rel=(data.data&&data.data.file)?data.data.file.split('/midias/').pop():''; if(rel) document.getElementById('source').value=rel;} else rel=document.getElementById('source').value; if(!rel) throw new Error('Escolha um vídeo da Biblioteca, selecione um arquivo ou cole um link.'); const p=await editorApi({action:'probe',source:rel}); if(!p.ok) throw new Error(p.message); CURRENT=p.data; document.getElementById('player').src=p.data.play_url||''; document.getElementById('captionSource').value=rel; document.getElementById('captionPlayer').src=p.data.play_url||''; box.textContent='Vídeo selecionado: '+JSON.stringify(p.data,null,2);}catch(err){box.textContent='Erro: '+err;}}
