...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='openclaw-cut-render-') as td:
        ...
        for i, k in enumerate(keeps):
            ...
            segs.append(seg)
            if args.copy:
                cmd = ['ffmpeg','-y','-hide_banner','-loglevel','error','-ss',f'{start:.3f}','-t',f'{dur:.3f}','-i',str(video),'-map','0:v:0','-map','0:a:0?','-c','copy','-avoid_negative_ts','make_zero',str(seg)]
            else:
                cmd = ['ffmpeg','-y','-hide_banner','-loglevel','error','-ss',f'{start:.3f}','-t',f'{dur:.3f}','-i',str(video),'-map','0:v:0','-map','0:a:0?']
                if args.scale:
        ...
        run(['ffmpeg','-y','-hide_banner','-loglevel','error','-f','concat','-safe','0','-i',str(concat),'-c','copy',str(args.output)])
    print(args.output)
    return 0
