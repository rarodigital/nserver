...
function setPlayerReady(url,msg){const st=document.getElementById('playerStatus'); const v=document.getElementById('player'); if(!v||!url)return; v.oncanplay=()=>{if(st){st.style.display='none';} v.style.opacity='1';}; v.onerror=()=>{if(st){st.style.display='block'; st.textContent='O navegador ainda está carregando o preview. Aguarde alguns segundos e tente reproduzir novamente.';} v.style.opacity='0.6';}; if(st){st.style.display='block'; st.textContent=msg||'Carregando vídeo no player...';} v.src=url; v.load();}
