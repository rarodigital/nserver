...
class VideoEditorProcessor(Processor):
    ...
    def analyze_cuts(self, source: Path, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        params = {
            "noise": str(payload.get("noise") or "-40dB"),
            "min_silence": float(payload.get("min_silence") or 0.75),
            "pad_before": float(payload.get("pad_before") or 0.30),
            "pad_after": float(payload.get("pad_after") or 0.30),
            "ignore_cut_under": float(payload.get("ignore_cut_under") or 0.20),
            "min_keep": float(payload.get("min_keep") or 0.25),
            "join_gap": float(payload.get("join_gap") or 0.15),
        }
        cmd = [
