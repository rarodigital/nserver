...
The defaults follow the original Kael CutFlow behavior: aggressive enough
for a first pass, then manually fine-tuned in the timeline.
"""
...
def build_cuts(video: Path, out: Path, noise: str, min_silence: float, pad_before: float, pad_after: float, min_keep: float, ignore_cut_under: float, join_gap: float) -> dict:
    ...
    data = {
        ...
        'basis': 'simple-silence-upload-v1-kael-compatible',
...
def main() -> int:
    ...
    ap.add_argument('--join-gap', type=float, default=0.0)
    args = ap.parse_args()
