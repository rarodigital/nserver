...
class CourseProcessor(Processor):
    ...
    def _lesson_markdown(self, course: dict[str, Any], module: dict[str, Any], lesson: dict[str, Any], transcript_text: str, video_url: str) -> str:
        return "\n".join([
            ...
            f"Aula ID: {lesson.get('id')}",
            f"URL da aula: {lesson.get('url')}",
            f"URL do vídeo: {video_url or '-'}", "",
            "## Transcrição", "", transcript_text.strip() or "[transcrição vazia]", "",
            "## Pontos-chave", "", "- [preencher/gerar depois]", "",
        ]) + "\n"
    ...
    def process(self, payload: dict[str, Any]) -> ProcessorResult:
        ...
        output = payload.get("output", "theroncore")  # video|audio|transcript|theroncore
        ...
        for n, (module, lesson) in enumerate(pairs, start=1):
            ...
            try:
                ...
                if output == "audio":
                    ...
                    item["files"].append(str(media_path))
                if output in {"transcript", "theroncore"}:
                    with tempfile.TemporaryDirectory() as td:
                        ...
                        # Copy generated transcript into the course package with stable name.
