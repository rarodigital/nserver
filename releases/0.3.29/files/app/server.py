        ...
        if path == "/tool/course-ingest":
            ...
            <main class="wrap">
              ...
              <section class="card" style="margin-top:18px">
                ...
                <input id="courseUrl" value="https://treinamentos.coreeducacao.com/area/vitrine/home" />
                <div class="row">
                  <div><label>Login</label><input id="courseEmail" placeholder="email do curso" /></div>
                  <div><label>Senha</label><input id="coursePassword" type="password" placeholder="senha" /></div>
                </div>
                <button onclick="mapCourse()">Mapear curso</button>
                <p class="mini">A senha é usada somente nesta execução; o Nserver não grava a senha em arquivo.</p>
                <div id="mapResult" class="result muted">Aguardando mapeamento.</div>
              </section>
              ...
              <section class="card" style="margin-top:18px">
                ...
                <div class="row">
                  <div><label>Saída</label><select id="output"><option value="theroncore">Pacote TheronCore: vídeo + transcrição organizada</option><option value="transcript">Somente transcrição</option><option value="audio">Somente áudio</option><option value="video">Somente vídeo</option></select></div>
                </div>
...
async function api(payload){const res=await fetch('/api/course',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); return await res.json();}
...
async function mapCourse(){
  ...
  try{box.textContent='Entrando no curso e mapeando módulos/aulas...'; const data=await api({action:'map',...creds()}); if(!data.ok) throw new Error(data.message); COURSE=data.data; box.textContent=data.message+'\nID: '+COURSE.id+'\nMódulos: '+COURSE.total_modules+'\nAulas: '+COURSE.total_lessons; renderCourse();}
...
async function processCourse(){
  ...
  try{box.textContent='Processando... pode demorar bastante dependendo da quantidade de aulas.'; const payload={action:'process',course_id:COURSE.id,scope:scope.value,module_id:moduleId.value,lesson_id:lessonId.value,output:output.value,...creds()}; const data=await api(payload); if(!data.ok) throw new Error(data.message); box.textContent=data.message+'\n'+JSON.stringify(data.data,null,2);}
