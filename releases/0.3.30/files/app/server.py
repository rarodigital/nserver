...
async function mapCourse(){
  ...
  try{box.textContent='Entrando no curso e mapeando módulos/aulas... se passar de 1 minuto, recarregue e tente novamente.'; const data=await api({action:'map',...creds()}); if(!data.ok) throw new Error(data.message); COURSE=data.data; box.textContent=data.message+'\nID: '+COURSE.id+'\nMódulos: '+COURSE.total_modules+'\nAulas: '+COURSE.total_lessons; renderCourse();}
