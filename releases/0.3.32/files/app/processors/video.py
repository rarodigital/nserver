...
from .media import MediaManager, VIDEO_EXTS, safe_name
...
def _yt_dlp_insert_index(args: list[str]) -> int:
    # For [python, -m, yt_dlp, ...], yt-dlp options must come after the module
    # name. Inserting at index 1 would pass them to python.exe itself.
...
def _run_ytdlp_with_retry(args: list[str], url: str, timeout: int = 3600) -> subprocess.CompletedProcess:
    proc = _run(args, timeout=timeout)
    if proc.returncode == 0 or not _needs_ytdlp_retry((proc.stderr or "") + (proc.stdout or "")):
        return proc
    first_error = ((proc.stderr or "") + "\n" + (proc.stdout or ""))[-1400:]
    upgrade_log = _upgrade_ytdlp_stack()
    base = _yt_dlp_base_cmd() or [args[0]]
    old_base_len = 3 if len(args) >= 3 and args[1:3] == ["-m", "yt_dlp"] else 1
    retry = [*base, *_tiktok_retry_args(url), *args[old_base_len:]]
    proc = _run(retry, timeout=timeout)
...
class VideoProcessor(Processor):
    ...
    def analyze(self, url: str) -> ProcessorResult:
        ...
        proc = _run_ytdlp_with_retry([*base, "--dump-single-json", "--no-playlist", url], url, timeout=180)
    ...
    def download_video(self, url: str, quality: str = "720", ext: str = "mp4", destination: str = "server") -> ProcessorResult:
        ...
        args = [*base, "--no-playlist", "-f", fmt, "--merge-output-format", ext, "--postprocessor-args", "Merger:-c:v copy -c:a aac", "--print", "after_move:filepath", "-o", outtmpl, url]
        ...
        try:
            file_path = self._make_mobile_compatible_mp4(file_path)
        except Exception as exc:
            return ProcessorResult(False, str(exc), {"folder": str(outdir), "file": str(file_path)})
        self._cleanup_intermediates(outdir, file_path)
        data = {"folder": str(outdir), "file": str(file_path), "filename": file_path.name, "size_bytes": file_path.stat().st_size, "downloadable": True, "delete_after_download": destination == "device", "compatibility": "MP4 H.264/AVC + AAC + yuv420p + faststart", "log": proc.stdout[-1000:]}
        if destination == "device":
            return ProcessorResult(True, "Vídeo pronto em MP4 compatível com TikTok/CapCut.", data)
        return ProcessorResult(True, "Vídeo salvo em MP4 compatível com TikTok/CapCut na Biblioteca.", data)
    ...
    def extract_audio(self, url: str, audio_format: str = "mp3", quality: str = "192", destination: str = "server") -> ProcessorResult:
        ...
        outdir.mkdir(parents=True, exist_ok=True)
        outtmpl = str(outdir / "%(title).90s-%(id)s.%(ext)s")
        args = [*base, "--no-playlist", "-x", "--audio-format", audio_format, "--audio-quality", quality + "K", "--print", "after_move:filepath", "-o", outtmpl, url]
        ffmpeg = _ffmpeg_location()
        if not ffmpeg:
    ...
    def _cleanup_intermediates(self, folder: Path, final_file: Path):
        ...
        for p in folder.rglob("*"):
            if not p.is_file() or p.resolve() == final_resolved:
                continue
            # Never delete existing library files when a new download finishes.
            # yt-dlp intermediates are either temporary suffixes or format shards
            # such as .f137.mp4. Older code deleted every other file in the
            # folder, which wiped the user's Biblioteca when downloading a new
            # video.
            if not (p.name.endswith((".part", ".ytdl")) or _is_intermediate_name(p)):
                continue
            try:
    ...
    def _make_mobile_compatible_mp4(self, file_path: Path) -> Path:
        """Normalize MP4 for TikTok/CapCut/mobile editors.

        Some sites deliver MP4 containers with HEVC, odd dimensions, non-yuv420p
        pixels or audio/layout metadata that browsers can play but mobile editors
        reject with "não foi possível descodificar". Re-encode only the final MP4
        into a conservative AVC/AAC file and replace it atomically.
        """
        ...
        if not ffmpeg:
            return file_path
        tmp = file_path.with_name(file_path.stem + ".compatible.tmp.mp4")
        cmd = [
            ffmpeg, "-y", "-i", str(file_path),
            "-map", "0:v:0", "-map", "0:a?",
            "-vf", "scale=trunc(iw/2)*2:trunc(ih/2)*2,format=yuv420p",
            "-c:v", "libx264", "-preset", "veryfast", "-crf", "23",
            "-profile:v", "high", "-level", "4.1", "-tag:v", "avc1",
        ...
        if proc.returncode != 0 or not tmp.exists() or tmp.stat().st_size <= 0:
            try:
                ...
                pass
            raise RuntimeError("Baixei o vídeo, mas falhei ao converter para MP4 compatível com TikTok/CapCut: " + ((proc.stderr or proc.stdout or "")[-900:]))
    ...
    def _download_audio_temp(self, url: str, tmp: Path) -> Path:
        ...
        proc = _run_ytdlp_with_retry([*base, "--no-playlist", "-f", "ba/bestaudio/b", "--print", "after_move:filepath", "-o", outtmpl, url], url, timeout=3600)
        if proc.returncode != 0:
