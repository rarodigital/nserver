...
class CourseProcessor(Processor):
    ...
    def map_course(self, url: str, email: str, password: str) -> ProcessorResult:
        try:
            return self._map_course_live(url, email, password)
        except Exception as exc:
            if self._is_core_course(url) and "timed out" in str(exc).lower():
                known = self._known_core_course_map(url)
                if known:
                    return ProcessorResult(True, "Curso mapeado pelo mapa rápido local porque o portal demorou para responder.", known)
            raise

    def _is_core_course(self, url: str) -> bool:
        return "treinamentos.coreeducacao.com" in (url or "").lower()

    def _known_core_course_map(self, url: str) -> dict[str, Any] | None:
        path = self.root / "system" / "core-course-known-map.json"
        if not path.exists():
            return None
        data = json.loads(path.read_text(encoding="utf-8"))
        data["id"] = "core-known-metodo-audience"
        data["source_url"] = url or data.get("source_url") or "https://treinamentos.coreeducacao.com/area/vitrine/home"
        data["mapped_at"] = time.strftime("%Y-%m-%d %H:%M:%S") + " (mapa rápido local)"
        state_path = self.state_root / f"{data['id']}.json"
    ...
    def _map_course_live(self, url: str, email: str, password: str) -> ProcessorResult:
