...
def _needs_ytdlp_retry(text: str) -> bool:
    low = (text or "").lower()
    return any(token in low for token in ["impersonation", "impersonate", "universal data", "rehydration", "confirm you are on the latest version", "tiktok"])


def _upgrade_ytdlp_stack() -> str:
    proc = subprocess.run(
        [sys.executable, "-m", "pip", "install", "--upgrade", "yt-dlp", "curl_cffi"],
        capture_output=True,
        text=True,
        timeout=600,
    )
...
def _tiktok_retry_args(url: str) -> list[str]:
    if "tiktok" not in (url or "").lower():
        return []
    # Recent TikTok extractor paths often need browser impersonation. This only
    # works when curl_cffi is installed; _upgrade_ytdlp_stack installs it.
    return ["--impersonate", "chrome"]


def _yt_dlp_insert_index(args: list[str]) -> int:
    return 3 if len(args) >= 3 and args[1:3] == ["-m", "yt_dlp"] else 1


def _add_ytdlp_global_args(args: list[str], extra: list[str]) -> list[str]:
    pos = _yt_dlp_insert_index(args)
    return [*args[:pos], *extra, *args[pos:]]
...
class VideoEditorProcessor(Processor):
    ...
    def run(self, payload: dict[str, Any]) -> ProcessorResult:
        ...
        try:
            ...
            if action == "prepare_url":
                source = self.prepare_url(str(payload.get("url") or ""), str(payload.get("quality") or "720"))
                return ProcessorResult(True, "Vídeo temporário carregado para edição. Ele não foi salvo na Biblioteca.", self.probe(source))
            if action == "auto_plan":
                source = self.resolve_media(payload.get("source", ""))
            ...
            if action == "analyze_cuts":
                source = self.resolve_media(payload.get("source", ""))
                data = self.analyze_cuts(source, payload)
                summary = data.get("summary") or {}
                message = "Silêncios analisados. Ajuste a timeline antes de exportar."
                if summary:
                    message = f"Silêncios analisados: {summary.get('cut_count', 0)} corte(s), {summary.get('removed_seconds', 0)}s removidos. Ajuste a timeline antes de exportar."
                return ProcessorResult(True, message, data)
            ...
            if action == "preview_cuts":
                source = self.resolve_media(payload.get("source", ""))
                data = self.preview_cuts(source, payload)
                return ProcessorResult(True, "Preview editado renderizado. O player agora mostra o resultado dos cortes.", data)
    ...
    def script_env(self) -> dict[str, str]:
        ...
        env["NSERVER_FFMPEG"] = ffmpeg
        ffprobe = self.ffprobe_cmd()
        if ffprobe:
            env["NSERVER_FFPROBE"] = ffprobe
        try:
    ...
    def prepare_url(self, url: str, quality: str = "720") -> Path:
        url = url.strip()
        if not url:
            raise ValueError("Cole o link do vídeo.")
        base = _yt_dlp_base_cmd()
        if not base:
            raise RuntimeError("yt-dlp não encontrado. Feche o Nserver e abra o iniciar-nserver.bat para instalar as dependências.")
        key = hashlib.sha1(url.encode("utf-8", errors="ignore")).hexdigest()[:16]
        folder = self.temp_dir / "url-sources" / key
        folder.mkdir(parents=True, exist_ok=True)
        existing = [p for p in folder.rglob("*") if p.is_file() and p.suffix.lower() in VIDEO_EXTS and not re.search(r"\.f\d+\.", p.name)]
        if existing:
            return max(existing, key=lambda p: p.stat().st_size)
        q = re.sub(r"[^0-9]", "", quality or "720") or "720"
        outtmpl = str(folder / "%(title).80s-%(id)s.%(ext)s")
        fmt = f"bv*[height<={q}]+ba[ext=m4a]/bv*[height<={q}]+ba/b[height<={q}][acodec!=none]/b[acodec!=none]/best[ext=mp4]/best"
        cmd = [*base, "--no-playlist", "-f", fmt, "--merge-output-format", "mp4", "--print", "after_move:filepath", "-o", outtmpl, url]
        ffmpeg = self.ffmpeg_cmd()
        if ffmpeg:
            cmd = _add_ytdlp_global_args(cmd, ["--ffmpeg-location", ffmpeg])
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=3600)
        if proc.returncode != 0 and _needs_ytdlp_retry((proc.stderr or "") + (proc.stdout or "")):
            first_error = ((proc.stderr or "") + "\n" + (proc.stdout or ""))[-1400:]
            upgrade_log = _upgrade_ytdlp_stack()
            base = _yt_dlp_base_cmd() or base
            old_base_len = 3 if len(cmd) >= 3 and cmd[1:3] == ["-m", "yt_dlp"] else 1
            retry = [*base, *_tiktok_retry_args(url), *cmd[old_base_len:]]
            proc = subprocess.run(retry, capture_output=True, text=True, timeout=3600)
            proc.stderr = "[primeira tentativa]\n" + first_error + "\n\n[auto-update yt-dlp/curl_cffi]\n" + upgrade_log + "\n\n[segunda tentativa]\n" + (proc.stderr or "")
        if proc.returncode != 0:
            msg = (proc.stderr or proc.stdout or "Não consegui carregar o link para edição.")[-900:]
            raise RuntimeError(msg)
        candidates = [Path(line.strip()) for line in (proc.stdout or "").splitlines() if line.strip()]
        for path in reversed(candidates):
            if path.exists() and path.is_file() and path.suffix.lower() in VIDEO_EXTS and not re.search(r"\.f\d+\.", path.name):
                return path
        files = [p for p in folder.rglob("*") if p.is_file() and p.suffix.lower() in VIDEO_EXTS and not re.search(r"\.f\d+\.", p.name)]
        if files:
            return max(files, key=lambda p: p.stat().st_size)
        raise RuntimeError("Vídeo temporário do link não foi encontrado.")
    ...
    def probe(self, source: Path) -> dict[str, Any]:
        ...
        if ffprobe:
            try:
                ...
                return self.ensure_browser_preview(source, info)
            except Exception:
                pass
    ...
    def ensure_browser_preview(self, source: Path, info: dict[str, Any]) -> dict[str, Any]:
        codec = str(info.get("video_codec") or "").lower()
        # TikTok commonly saves MP4 as HEVC/H.265. The file is valid, but many
        # Windows browser players do not render HEVC in <video>. Keep the
        # original as the editor source and create a browser-safe H.264 proxy
        # only for playback/preview.
        if codec not in {"hevc", "h265", "h.265"}:
            return info
        proxy_dir = self.temp_dir / "proxies"
        proxy_dir.mkdir(parents=True, exist_ok=True)
        stat = source.stat()
        proxy = proxy_dir / f"{self.slug(source.stem)}-{stat.st_size}-{int(stat.st_mtime)}-h264-preview-v2.mp4"
        if not proxy.exists() or proxy.stat().st_size == 0:
            cmd = [
                self.ffmpeg_cmd(), "-y", "-i", str(source),
                "-map", "0:v:0", "-map", "0:a?",
        ...
        info["preview_source"] = proxy.relative_to(self.media_root).as_posix()
        info["preview_note"] = "Preview convertido automaticamente para H.264 porque o original está em HEVC/H.265. O arquivo original continua sendo a fonte do editor."
        return info
    ...
    def analyze_cuts(self, source: Path, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        params = {
            # Default back to the Kael CutFlow behavior. It was more aggressive
            # and then expects manual fine-tuning, which matches the user's flow.
            ...
            "join_gap": float(payload.get("join_gap") or 0.0),
        }
        cmd = [
            sys.executable, str(script), str(source), "-o", str(cuts_path),
            ...
            f"--join-gap={params['join_gap']}",
        ]
        subprocess.run(cmd, check=True, capture_output=True, text=True, env=self.script_env())
        cuts = json.loads(cuts_path.read_text(encoding="utf-8"))
        cut_items = [x for x in cuts.get("items") or [] if x.get("action") == "cut"]
        keep_items = [x for x in cuts.get("items") or [] if x.get("action") == "keep"]
        removed = round(sum(float(x.get("end", 0)) - float(x.get("start", 0)) for x in cut_items), 3)
        kept = round(sum(float(x.get("end", 0)) - float(x.get("start", 0)) for x in keep_items), 3)
        summary = {
            "cut_count": len(cut_items),
            "keep_count": len(keep_items),
            "removed_seconds": removed,
            "kept_seconds": kept,
            "removed_percent": round((removed / float(cuts.get("source_duration") or 1)) * 100, 1),
        }
        cuts["summary"] = {**(cuts.get("summary") or {}), **summary}
        cuts_path.write_text(json.dumps(cuts, ensure_ascii=False, indent=2), encoding="utf-8")
        return {"cuts": cuts, "cuts_path": str(cuts_path), "segments": self.cuts_to_segments(cuts), "summary": summary, **self.probe(source)}
    ...
    def render_cuts(self, source: Path, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        subprocess.run([sys.executable, str(script), str(cuts_path), "--video", str(source), "-o", str(out)], check=True, capture_output=True, text=True, env=self.script_env())
        return {"file": str(out), "relative": out.relative_to(self.media_root).as_posix(), "filename": out.name, "cuts_path": str(cuts_path)}

    def preview_cuts(self, source: Path, payload: dict[str, Any]) -> dict[str, Any]:
        cuts = payload.get("cuts") or {}
        if not cuts.get("items"):
            raise ValueError("Nenhum cuts.json válido recebido para preview.")
        stamp = time.strftime("%Y%m%d-%H%M%S")
        title = self.slug(payload.get("title") or source.stem)
        cuts["source_video"] = str(source)
        preview_dir = self.temp_dir / "previews"
        preview_dir.mkdir(parents=True, exist_ok=True)
        cuts_path = self.projects_dir / f"{title}-preview-{stamp}.cuts.json"
        out = preview_dir / f"{title}-cutflow-preview-{stamp}.mp4"
        cuts_path.write_text(json.dumps(cuts, ensure_ascii=False, indent=2), encoding="utf-8")
        script = self.root / "tools" / "cut-editor" / "render_from_cuts.py"
        subprocess.run([
            sys.executable, str(script), str(cuts_path), "--video", str(source), "-o", str(out),
            "--preset", "ultrafast", "--crf", "28", "--scale", "min(960\\,iw):-2", "--fps", "30",
        ], check=True, capture_output=True, text=True, env=self.script_env())
        return {"file": str(out), "relative": out.relative_to(self.media_root).as_posix(), "filename": out.name, "cuts_path": str(cuts_path), "preview": True}
    ...
    def caption_render(self, source: Path, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        subprocess.run([sys.executable, str(script), str(source), str(cfg_path), "-o", str(out)], check=True, capture_output=True, text=True, env=self.script_env())
