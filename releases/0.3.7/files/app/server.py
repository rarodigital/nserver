  ...
  try {{
    ...
    if (btn) btn.disabled = false;
        ...
        if path in {"/tool/transcription", "/tool/viral-clips"}:
            ...
            description = "Gere transcrições em TXT, MD, DOCX ou PDF usando modo local gratuito ou OpenAI opcional." if is_transcription else "Crie cortes virais estilo OpusClip. Esta fase gera plano/ranking; a próxima conecta renderização automática."
            if is_transcription:
                actions_html = """
                  <div>
                    <h3>Transcrição</h3>
                ...
                actions_html = """
                  <div>
                    <h3>Cortes virais</h3>
                    <p class="mini">Informe quantidade e duração. O Nserver vai analisar, sugerir trechos e dar nota de potencial viral. A renderização automática vem na próxima fase.</p>
...
async function analyzeVideo() {{ const box=document.getElementById('result'); const btn=event?.target; try {{ if(!currentUrl()){{box.textContent='Cole um link primeiro.';return;}} if(btn)btn.disabled=true; box.textContent='Analisando...'; document.getElementById('info').innerHTML=''; const data=await api({{action:'analyze',url:currentUrl()}}); box.textContent=data.message||'Resposta recebida.'; if(data.ok&&data.data){{const d=data.data; document.getElementById('info').innerHTML=`<div class="video-info">${{d.thumbnail?`<img src="${{esc(d.thumbnail)}}">`:''}}<div><h2>${{esc(d.title)}}</h2><p><b>Plataforma:</b> ${{esc(d.platform)}}<br><b>Duração:</b> ${{esc(d.duration_text)}}<br><b>Resoluções:</b> ${{esc((d.resolutions||[]).map(x=>x.label).join(', ')||'não informado')}}</p></div></div>`;}} else if(data.data&&data.data.details){{box.textContent+='\\nDetalhes: '+data.data.details;}} }} catch(err) {{ box.textContent='Erro ao analisar: '+err; }} finally {{ if(btn)btn.disabled=false; }} }}
async function runAction(action) {{ const box=document.getElementById('result'); const btn=event?.target; try {{ if(!currentUrl()){{box.textContent='Cole um link primeiro.';return;}} if(btn)btn.disabled=true; box.textContent='Processando... isso pode levar alguns minutos.'; const payload={{action,url:currentUrl()}}; if(action==='transcribe'){{payload.formats=[...document.querySelectorAll('.transcript-format:checked')].map(x=>x.value);}} if(action==='viral_clips'){{payload.count=document.getElementById('clipCount').value; payload.max_seconds=document.getElementById('clipSeconds').value;}} const data=await api(payload); box.textContent=(data.message||'Resposta recebida.')+(data.data?'\\n'+JSON.stringify(data.data,null,2):''); await loadHistory(); }} catch(err) {{ box.textContent='Erro ao processar. Detalhes: '+err; }} finally {{ if(btn)btn.disabled=false; }} }}
        ...
        if path == "/tool/video-editor":
            ...
            <main class="wrap">
              <section class="card">
                ...
                <p>Editor inspirado no fluxo do Kael/video-use: escolha um vídeo da Biblioteca, monte trechos, gere preview rápido e depois renderize o final.</p>
                <label>Vídeo da Biblioteca</label>
                <div class="row">
                  <select id="source"></select>
                  <button onclick="probeSource()">Analisar</button>
                </div>
...
async function loadSources(){const data=await editorApi({action:'list_sources'}); SOURCES=(data.data&&data.data.items)||[]; const sel=document.getElementById('source'); sel.innerHTML=SOURCES.map(x=>`<option value="${esc(x.relative)}">${esc(x.name)} — ${esc(x.modified)}</option>`).join(''); document.getElementById('result').textContent=SOURCES.length?'Escolha um vídeo e clique em Analisar.':'Nenhum vídeo encontrado na Biblioteca. Baixe ou salve um vídeo primeiro.';}
