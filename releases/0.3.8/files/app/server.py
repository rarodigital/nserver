...
from processors.media import MediaManager
...
PROCESSORS = {"video-downloader": VideoProcessor(ROOT), "video-editor": VideoEditorProcessor(ROOT)}
...
def media_kind(path: Path) -> str:
    ...
    if rel == "Videos" or low in {".mp4", ".mov", ".mkv", ".webm"}:
        return "Vídeos"
    if rel == "Audios" or low in {".mp3", ".wav", ".m4a", ".ogg", ".aac"}:
        return "Áudios"
    ...
    for path in MIDIAS.rglob("*"):
        ...
        try:
            ...
            items.append({
                ...
                "is_video": resolved.suffix.lower() in {".mp4", ".webm", ".mov", ".mkv"},
                "is_audio": resolved.suffix.lower() in {".mp3", ".wav", ".m4a", ".ogg", ".aac"},
...
loadSettings(); loadHistory(); loadMediaOptions(); toggleSourceInputs();
        ...
        if path == "/tool/video-editor":
            sess = self.require_login()
            if not sess: return
            body = html_page("Editor de Vídeo", """
            <main class="wrap">
              <section class="card">
                <h1>Ferramenta 04 — Editor de Vídeo</h1>
                <p>Editor inspirado no CutFlow/video-use: selecione mídia, monte cortes em timeline, gere preview e exporte o final.</p>
                <h2>Etapa 1 — Selecionar vídeo</h2>
                <label>Fonte</label>
                <div class="row">
                  <select id="sourceType" onchange="toggleEditorSource()"><option value="library">Biblioteca</option><option value="upload">Upload</option><option value="url">URL</option></select>
                  <select id="source"></select>
                  <input id="editorUpload" type="file" style="display:none" />
                  <input id="editorUrl" placeholder="https://youtube.com/..." style="display:none" />
                  <button onclick="prepareEditorSource()">Analisar</button>
                </div>
                <div id="result" class="result muted">Carregando vídeos disponíveis...</div>
              </section>

              <section class="card" style="margin-top:18px">
                <h2>Timeline de cortes</h2>
                <p class="mini">Cada linha é um trecho que entra no vídeo final. O original não é alterado.</p>
                <div class="row">
                  <button onclick="autoPlan()">Criar timeline automática</button>
                  <button onclick="addSegment()">Adicionar trecho</button>
                  <button class="danger-btn" onclick="clearSegments()">Limpar timeline</button>
                </div>
                <div id="segments" class="history-list"></div>
              </section>

              <section class="card" style="margin-top:18px">
                <h2>Renderização</h2>
                <div class="row">
                  <div><label>Nome do projeto</label><input id="title" placeholder="meu-video-editado" /></div>
                  <div><label>Formato</label><select id="aspect"><option value="original" selected>Original</option><option value="16:9">16:9 YouTube</option><option value="9:16">9:16 Reels/Shorts</option><option value="1:1">1:1 Quadrado</option></select></div>
                  <div><label>Grade</label><select id="grade"><option value="none" selected>Sem grade</option><option value="punch">Contraste leve</option><option value="warm">Quente cinematográfico</option></select></div>
                </div>
                <div class="row">
                  <button onclick="renderEdit('preview')">Renderizar preview rápido</button>
                  <button onclick="renderEdit('final')">Renderizar final</button>
                </div>
              </section>
            </main>
<script>
let SOURCES=[]; let CURRENT=null; let SEGMENTS=[];
function esc(value){return String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));}
async function editorApi(payload){const res=await fetch('/api/editor',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});return await res.json();}
async function videoApi(payload){const res=await fetch('/api/video',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});return await res.json();}
async function uploadEditorFile(){const input=document.getElementById('editorUpload'); if(!input.files.length) throw new Error('Escolha um vídeo para upload.'); const fd=new FormData(); fd.append('file',input.files[0]); const res=await fetch('/api/media',{method:'POST',body:fd}); const data=await res.json(); if(!data.ok) throw new Error(data.message||'Falha no upload.'); await loadSources(); return data.data.relative;}
function toggleEditorSource(){const t=document.getElementById('sourceType').value; document.getElementById('source').style.display=t==='library'?'block':'none'; document.getElementById('editorUpload').style.display=t==='upload'?'block':'none'; document.getElementById('editorUrl').style.display=t==='url'?'block':'none';}
async function prepareEditorSource(){const t=document.getElementById('sourceType').value; const box=document.getElementById('result'); try{ if(t==='upload'){const rel=await uploadEditorFile(); document.getElementById('sourceType').value='library'; toggleEditorSource(); document.getElementById('source').value=rel;} if(t==='url'){box.textContent='Baixando vídeo para a Biblioteca...'; const data=await videoApi({action:'download_video',url:document.getElementById('editorUrl').value,destination:'server',quality:'720',format:'mp4'}); if(!data.ok) throw new Error(data.message); await loadSources(); const rel=(data.data&&data.data.file)?data.data.file.split('/midias/').pop():''; if(rel) document.getElementById('source').value=rel; document.getElementById('sourceType').value='library'; toggleEditorSource();} await probeSource(); }catch(err){box.textContent='Erro ao preparar vídeo: '+err;}}
async function loadSources(){const data=await editorApi({action:'list_sources'}); SOURCES=(data.data&&data.data.items)||[]; const sel=document.getElementById('source'); sel.innerHTML=SOURCES.map(x=>`<option value="${esc(x.relative)}">${esc(x.name)} — ${esc(x.modified)}</option>`).join(''); document.getElementById('result').textContent=SOURCES.length?'Escolha um vídeo e clique em Analisar.':'Nenhum vídeo encontrado na Biblioteca. Baixe, envie upload ou informe uma URL.';}
async function probeSource(){const source=document.getElementById('source').value; if(!source){return;} const data=await editorApi({action:'probe',source}); CURRENT=data.data; document.getElementById('title').value=(CURRENT.filename||'video').replace(/\\.[^.]+$/,'')+'-editado'; document.getElementById('result').textContent=(data.message||'Ok')+'\n'+JSON.stringify(CURRENT,null,2);}
function renderSegments(){const box=document.getElementById('segments'); box.innerHTML=SEGMENTS.map((s,i)=>`<div class="history-item"><div class="thumb" style="display:grid;place-items:center">#${i+1}</div><div><div class="row"><div><label>Início (s)</label><input type="number" step="0.01" value="${s.start}" onchange="SEGMENTS[${i}].start=parseFloat(this.value||0)"></div><div><label>Fim (s)</label><input type="number" step="0.01" value="${s.end}" onchange="SEGMENTS[${i}].end=parseFloat(this.value||0)"></div><div><label>Rótulo</label><input value="${esc(s.label||'')}" onchange="SEGMENTS[${i}].label=this.value"></div></div><button class="danger-btn" onclick="SEGMENTS.splice(${i},1);renderSegments()">Remover trecho</button></div></div>`).join('') || '<p class="muted">Nenhum trecho ainda. Crie uma timeline automática ou adicione manualmente.</p>';}
async function autoPlan(){if(!CURRENT) await probeSource(); if(!document.getElementById('source').value)return; const data=await editorApi({action:'auto_plan',source:document.getElementById('source').value,segment_seconds:30}); SEGMENTS=(data.data&&data.data.segments)||[]; document.getElementById('result').textContent=data.message; renderSegments();}
function addSegment(){const end=CURRENT&&CURRENT.duration?Math.min(30,CURRENT.duration):30; SEGMENTS.push({start:0,end,label:'Novo trecho'}); renderSegments();}
function clearSegments(){if(confirm('Limpar todos os trechos da timeline?')){SEGMENTS=[];renderSegments();}}
async function renderEdit(mode){const box=document.getElementById('result'); const source=document.getElementById('source').value; if(!source){box.textContent='Escolha um vídeo primeiro.';return;} if(!SEGMENTS.length){box.textContent='Adicione pelo menos um trecho na timeline.';return;} box.textContent='Renderizando... isso pode levar alguns minutos.'; const payload={action:'render',source,segments:SEGMENTS,mode,title:document.getElementById('title').value,aspect:document.getElementById('aspect').value,grade:document.getElementById('grade').value}; const data=await editorApi(payload); box.textContent=(data.message||'Resposta recebida.')+(data.data?'\n'+JSON.stringify(data.data,null,2):'');}
loadSources(); renderSegments(); toggleEditorSource();
</script>
            """, authenticated=True)
            self.send_html(body)
            return
        ...
        if path.startswith("/download/"):
            ...
            try:
                ...
                if size <= 0:
                    ...
                    body = b"Arquivo ainda nao esta pronto ou ficou vazio. Tente processar novamente."
                ...
                if item.get("delete_after_download"):
                    def cleanup_download(path=resolved, dl_token=token):
                        time.sleep(180)
                        try:
                            path.unlink(missing_ok=True)
                            parent = path.parent
                            if parent.name.startswith(("video-", "audio-")):
                                shutil.rmtree(parent, ignore_errors=True)
                            DOWNLOADS.pop(dl_token, None)
                        except Exception:
                            pass
        ...
        if path == "/library":
            ...
            <main class="wrap">
              <section class="card">
                <h1>Biblioteca</h1>
                <p>Arquivos armazenados no Nserver: vídeos, áudios, transcrições e cortes.</p>
                <div id="stats" class="stats"></div>
                <div class="row">
                  <input id="mediaSearch" placeholder="Buscar por nome..." oninput="renderMedia()" />
                  <select id="kindFilter" onchange="renderMedia()"><option value="">Todos</option><option>Vídeos</option><option>Áudios</option><option>Transcrições</option><option>Cortes</option></select>
                  <select id="sortFilter" onchange="renderMedia()"><option value="date">Mais recentes</option><option value="size">Maior tamanho</option><option value="name">Nome</option></select>
                  <label class="mini"><input type="checkbox" id="favoriteOnly" onchange="renderMedia()"> Mostrar apenas favoritos</label>
                </div>
                <div id="reader" class="result" style="display:none"></div>
...
let MEDIA=[];
function esc(value){return String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));}
async function libraryApi(payload){const res=await fetch('/api/library',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});return await res.json();}
async function loadLibrary(){const data=await libraryApi({action:'list'}); MEDIA=data.data.items||[]; renderStats(data.data.stats||{}); renderMedia();}
function renderStats(st){document.getElementById('stats').innerHTML=['Vídeos','Áudios','Transcrições','Cortes'].map(k=>`<div class="stat"><b>${k}</b><br>${st[k]||0}</div>`).join('')+`<div class="stat"><b>Utilizado</b><br>${esc(st.size||'0 B')}</div><div class="stat"><b>Disponível</b><br>${esc(st.disk_free||'-')}</div><div class="stat"><b>Total do disco</b><br>${esc(st.disk_total||'-')}</div>`;}
function renderMedia(){let q=document.getElementById('mediaSearch').value.toLowerCase();let kind=document.getElementById('kindFilter').value;let sort=document.getElementById('sortFilter').value;let fav=document.getElementById('favoriteOnly').checked;let items=MEDIA.filter(x=>(!kind||x.kind===kind)&&(!fav||x.favorite)&&(`${x.name} ${x.kind} ${x.relative}`.toLowerCase().includes(q)));if(sort==='size')items.sort((a,b)=>b.size_bytes-a.size_bytes);if(sort==='name')items.sort((a,b)=>a.name.localeCompare(b.name));let box=document.getElementById('mediaList');box.innerHTML=items.map(x=>`<div class="media-item" id="media-${esc(x.relative).replace(/[^a-zA-Z0-9]/g,'_')}"><div>${x.is_video?`<video class="thumb" src="${x.play_url}" muted></video>`:'<div class="thumb"></div>'}</div><div><h3><button onclick="toggleFavorite('${esc(x.relative)}')" title="Favoritar">${x.favorite?'★':'☆'}</button> ${esc(x.name)}</h3><p class="mini">${esc(x.kind)} • ${esc(x.size)} • ${esc(x.modified)}<br>${esc(x.relative)}</p>${x.is_video?`<video class="player" controls src="${x.play_url}"></video>`:x.is_audio?`<audio controls src="${x.play_url}"></audio>`:''}<div class="item-actions">${x.is_text?`<button onclick="openText('${esc(x.relative)}')">Abrir</button>`:''}<a class="button secondary" href="${x.download_url}" download>Baixar</a><button onclick="openFolder('${esc(x.relative)}')">Abrir pasta</button><button class="danger-btn" onclick="deleteMedia('${esc(x.relative)}')">Excluir arquivo</button></div></div></div>`).join('')||'<p class="muted">Nenhum arquivo encontrado.</p>';}
async function openFolder(rel){const data=await libraryApi({action:'open_folder', relative:rel}); alert(data.message);}
async function toggleFavorite(rel){await libraryApi({action:'favorite', relative:rel}); await loadLibrary();}
async function deleteMedia(rel){if(!confirm('Excluir este arquivo da Biblioteca e da pasta do PC? Essa ação não apaga apenas o histórico, ela remove o arquivo físico.'))return;const data=await libraryApi({action:'delete_file', relative:rel});alert(data.message);await loadLibrary();}
async function openText(rel){const data=await libraryApi({action:'read_text', relative:rel});const r=document.getElementById('reader');r.style.display='block';r.innerHTML=`<div class="row"><input id="readerSearch" placeholder="Buscar na transcrição..." oninput="filterReader()"><button onclick="navigator.clipboard.writeText(document.getElementById('readerText').innerText);alert('Texto copiado.')">Copiar texto completo</button><button onclick="r.requestFullscreen && r.requestFullscreen()">Tela cheia</button><button onclick="r.style.display='none'">Fechar</button></div><pre id="readerText" style="white-space:pre-wrap;max-height:70vh;overflow:auto">${esc(data.data.text||'')}</pre>`;}
function filterReader(){let q=document.getElementById('readerSearch').value.toLowerCase();let pre=document.getElementById('readerText');let raw=pre.innerText;if(!q){pre.innerHTML=esc(raw);return;}pre.innerHTML=esc(raw).replaceAll(esc(q),`<mark>${esc(q)}</mark>`);}
loadLibrary();
</script>
            """, authenticated=True)
            self.send_html(body)
...
async function restartServer() {{
  if(!confirm('Reiniciar o Nserver agora? A página pode ficar fora do ar por alguns segundos.')) return;
  const box=document.getElementById('updateResult');
  box.textContent='Reiniciando o Nserver... aguarde alguns segundos e recarregue a página.';
  const data = await updateApi({{action:'restart'}});
  box.textContent = data.message + '\\n' + JSON.stringify(data.data || {{}}, null, 2);
  setTimeout(() => location.reload(), 5000);
    ...
    def do_POST(self):
        ...
        if path == "/api/editor":
            ...
            data = result.data or {}
            if result.ok and payload.get("action") == "render" and data.get("file"):
                upsert_history({
